...
void adc_init_pad(void) {
    ...
    //ADC1->RSQR1 = 0;
    ADC1->RSQR1 = (2-1) << 20; // 2 conversions
    ADC1->RSQR2 = 0;
    ADC1->RSQR3 = (channel << 0) | (channel2 << 5); // 0-9 for 8 ext inputs and two internals

    // set sampling time for chl 3
    ADC1->SAMPTR2 &= ~(ADC_SMP0 << (3 * channel)) & ~(7 << (3 * channel2));
    ADC1->SAMPTR2 |= 7 << (3 * channel); // 0:7 => 3/9/15/30/43/57/73/241 cycles

    // set sampling time for chl 6
    ADC1->SAMPTR2 |= 7 << (3 * channel2); // 0:7 => 3/9/15/30/43/57/73/241 cycles
...
void adc_get_pad(uint16_t * val, uint16_t * val2) {
...
int main(void) {
    ...
    while (1) {
        ...
        uint16_t val, val2;
        adc_get_pad(&val, &val2);
        printf("ADC reading: %d, %d;", val, val2);
