...
#define rv_iimm_b(i)                                                           \
    (rv_signext(rv_tb(i, 31, 12), 12) | rv_tb(i, 7, 11) |                      \
        rv_tbf(i, 30, 25, 5) | rv_tbf(i, 11, 8, 1)) /* imm. for B-type */
#define rv_iimm_j(i)                                                           \
    (rv_signext(rv_tb(i, 31, 20), 20) | rv_tbf(i, 19, 12, 12) |                \
        rv_tb(i, 20, 11) | rv_tbf(i, 30, 21, 1)) /* imm. for J-type */
#define rv_isz(i)                                                              \
    (rv_bf(i, 1, 0) == 3 ? 4 : 2) /* instruction size                          \
                                   */
...
static rv_res rv_csr_bus(rv * cpu, rv_u32 csr, rv_u32 w, rv_u32 * io) {
    rv_u32 *y = NULL /* phys. register */, wm /* writable bits */ = -1U,
           rm = -1U;
...
static rv_u32 rv_trap(rv * cpu, rv_u32 cause, rv_u32 tval) {
    ...
    rv_priv xp = /* destination privilege, switch from y = cpu->priv
                    to this */
        (cpu->priv < RV_PMACH) &&
    ...
    if (xp == RV_PSUPER) /* select s-mode regs */
        xtvec = &cpu->csr.stvec, xepc = &cpu->csr.sepc,
        xcause = &cpu->csr.scause, xtval = &cpu->csr.stval;
    cpu->csr.mstatus &=
        (xp == RV_PMACH ? 0xFFFFE777   /* {mpp, mie, mpie} <- 0 */
    ...
    if (!rv_b(cpu->csr.satp, 31) || epriv > RV_PSUPER) {
        *pa = va; /* if !satp.mode, no translation */
        ...
        rv_u32 ppn /* satp.ppn */ = rv_bf(cpu->csr.satp, 21, 0),
                   a /* satp.ppn * PAGESIZE */ = ppn << 12,
                   i /* LEVELS - 1 */ = 1, pte, pte_address, tlb_hit = 0;
        ...
        while (!tlb_hit) {
            ...
            if (!rv_b(pte, 0) || (!rv_b(pte, 1) && rv_b(pte, 2)))
                return RV_PAGEFAULT; /* pte.v == 0, or (pte.r == 0 and
                                        pte.w == 1) */
            if (rv_b(pte, 1) || rv_b(pte, 3))
                break; /* if pte.r = 1 or pte.x = 1, this is a leaf
                          page */
            if (i == 0)
        ...
        *pa = rv_tbf(pte, 31, 10 + 10 * i, 12 + 10 * i) |
              rv_bf(va, 11 + 10 * i, 0);
    }
    return RV_OK;
}
...
#define rvc_imm_ciw(c)                     /* CIW imm. for c.addi4spn */       \
    (rv_tbf(c, 10, 7, 6) | rv_tbf(c, 12, 11, 4) | rv_tb(c, 6, 2) |             \
        rv_tb(c, 5, 3))
...
#define rvc_imm_cj(c) /* CJ imm. for c.jalr/c.j */                             \
    (rv_signext(rv_tb(c, 12, 11), 11) | rv_tb(c, 11, 4) |                      \
        rv_tbf(c, 10, 9, 8) | rv_tb(c, 8, 10) | rv_tb(c, 7, 6) |               \
...
#define rv_i_b(op, f3, rs1, rs2, imm) /* B-type */                             \
    (rv_b(imm, 12) << 31 | rv_bf(imm, 10, 5) << 25 | (rs2) << 20 |             \
        (rs1) << 15 | (f3) << 12 | rv_bf(imm, 4, 1) << 8 |                     \
    ...
    if (rvc_op(c) == 0) {
        ...
        if (rvc_f3(c) == 0) { /*I c.addi -> addi rd, rd, nzimm */
            ...
            if (rv_bf(c, 11, 10) == 0) { /*I c.srli -> srli rd', rd', shamt */
                return rv_i_r(
                    4, 5, rvc_irph(c), rvc_irph(c), rvc_imm_ci(c) & 0x1F, 0);
            }
            else if (rv_bf(c, 11, 10) ==
                     1) { /*I c.srai -> srai rd', rd', shamt */
                return rv_i_r(
                    4, 5, rvc_irph(c), rvc_irph(c), rvc_imm_ci(c) & 0x1F, 32);
            }
            else if (rv_bf(c, 11, 10) ==
                     2) { /*I c.andi -> andi rd', rd', imm */
                return rv_i_i(4, 7, rvc_irph(c), rvc_irph(c), rvc_imm_ci(c));
            }
            else if (rv_bf(c, 11, 10) == 3) {
                if (rv_bf(c, 6, 5) == 0) { /*I c.sub -> sub rd', rd', rs2' */
                    return rv_i_r(
                        12, 0, rvc_irph(c), rvc_irph(c), rvc_irpl(c), 32);
                }
                else if (rv_bf(c, 6, 5) ==
                         1) { /*I c.xor -> xor rd', rd', rs2' */
                    return rv_i_r(
                        12, 4, rvc_irph(c), rvc_irph(c), rvc_irpl(c), 0);
                }
                else if (rv_bf(c, 6, 5) == 2) { /*I c.or -> or rd', rd', rs2' */
                    return rv_i_r(
                        12, 6, rvc_irph(c), rvc_irph(c), rvc_irpl(c), 0);
                }
                else if (rv_bf(c, 6, 5) ==
                         3) { /*I c.and -> and rd', rd', rs2' */
                    return rv_i_r(
                        12, 7, rvc_irph(c), rvc_irph(c), rvc_irpl(c), 0);
                }
                else { /* illegal */
                    return 0;
                }
        ...
        if (rvc_f3(c) == 0) { /*I c.slli -> slli rd, rd, shamt */
            return rv_i_r(
                4, 1, rvc_ird(c), rvc_ird(c), rvc_imm_ci(c) & 0x1F, 0);
        }
            ...
            return rv_i_i(25, 0, 0, rvc_ird(c), 0);
                 ...
                 !rv_b(c, 12)) { /*I c.mv -> add rd, x0, rs2 */
...
void rv_endcvt(rv_u8 * in, rv_u8 * out, rv_u32 width, rv_u32 is_store) {
    if (!is_store && width == 1)
        ...
        *((rv_u32 *)out) = (rv_u32)(in[0] << 0) | (rv_u32)(in[1] << 8) |
        ...
        out[2] = *(rv_u32 *)in >> 16 & 0xFF,
        out[3] = *(rv_u32 *)in >> 24 & 0xFF;
}

/* perform a bus access. access == RV_AW stores data. */
static rv_u32 rv_bus(
    rv * cpu, rv_u32 * va, rv_u8 * data, rv_u32 width, rv_access access) {
    ...
    {
        rv_u32 w0 /* load this many bytes from 1st page */ =
            0x1000 - (*va & 0xFFF);
...
static rv_u32 rv_if(rv * cpu, rv_u32 * i, rv_u32 * tval) {
    ...
    if (cpu->pc & 2 || page) { /* perform fetch in two 2-byte fetches */
        ...
        if ((err = rv_bus(
                 cpu, &pc, (rv_u8 *)&ia, 2, RV_AX))) /* fetch 1st half */
            goto error;
        if (rv_isz(ia) == 4 &&
            (pc += 2, 1) && /* if instruction is 4 byte wide */
            (err = rv_bus(
                 cpu, &pc, (rv_u8 *)&ib, 2, RV_AX))) /* fetch 2nd half */
            goto error; /* need pc += 2 above for accurate {page}fault
                           traps */
...
rv_u32 rv_step(rv * cpu) {
    ...
    if (rv_iopl(i) == 0) {
        if (rv_ioph(i) == 0) { /*Q 00/000: LOAD */
            rv_u32 va /* virtual address */ =
                rv_lr(cpu, rv_irs1(i)) + rv_iimm_i(i);
            rv_u32 v /* loaded value */ = 0, w /* value width */,
                     sx /* sign ext. */;
            w = 1 << (rv_if3(i) & 3),
            sx = ~rv_if3(i) & 4; /*I lb, lh, lw, lbu, lhu */
            if ((err = rv_bus(cpu, &va, (rv_u8 *)&v, w, RV_AR)))
                return rv_trap_bus(cpu, err, va, RV_AR);
            if ((rv_if3(i) & 3) == 3)
                return rv_trap(
                    cpu, RV_EILL, tval); /* ld instruction not supported */
            if (sx)
            ...
            rv_u32 zero = !y, sgn = rv_sgn(y), ovf = rv_ovf(a, b, y),
                   carry = y > a;
        ...
        if (rv_ioph(i) == 0) {                /*Q 00/011: MISC-MEM */
            ...
            rv_u32 l /* should load? */ = rv_if5(i) != 3,
                     s /* should store? */ = 1;
            if (rv_bf(i, 14, 12) != 2) { /* width must be 2 */
                ...
                if (rv_if5(i) == 0) /*I amoadd.w */
                    ...
                    x = !(cpu->res_valid && cpu->res_valid-- && cpu->res == va),
                    s = !x;
        ...
        if (rv_ioph(i) == 0 ||  /*Q 00/100: OP-IMM */
            ...
            if (!rv_ioph(i) || !rv_b(i, 25)) {
                ...
                rv_u32 as /* sgn(a) */ = 0, bs /* sgn(b) */ = 0, ylo,
                          yhi /* result */;
                if (rv_if3(i) < 4) { /*I mul, mulh, mulhsu, mulhu */
                    if (rv_if3(i) < 3 &&
                        rv_sgn(a)) /* a is signed iff f3 in {0, 1, 2} */
                        a = ~a + 1, as = 1; /* two's complement */
                    if (rv_if3(i) < 2 &&
                        rv_sgn(b))          /* b is signed iff f3 in {0, 1} */
                        b = ~b + 1, bs = 1; /* two's complement */
                    ylo = rvm(a, b, &yhi);  /* perform multiply */
                    if (as != bs) /* invert output quantity if result <0 */
                        ylo = ~ylo + 1,
                        yhi = ~yhi + !ylo;     /* two's complement */
                    y = rv_if3(i) ? yhi : ylo; /* return hi word if mulh,
                                                  otherwise lo */
                }
                else {
                    if (rv_if3(i) == 4) /*I div */
                        ...
                        y = a % b;
                } /* all this because we don't have 64bits. worth it?
                     probably not B) */
            }
            ...
            rv_u32 s /* uimm */ =
                rv_if3(i) & 4 ? rv_irs1(i) : rv_lr(cpu, rv_irs1(i));
            if ((rv_if3(i) & 3) == 1) {              /*I csrrw, csrrwi */
                ...
                if (rv_irs1(i) &&
                    rv_csr_bus(cpu, csr, 1, &y)) /*     s|y into CSR */
                    return rv_trap(cpu, RV_EILL, tval);
            }
                ...
                if (!rv_ird(i)) {
                    if (!rv_irs1(i) && rv_irs2(i) == 2 &&
                        ...
                        rv_u32 yp /* previous (incoming) privilege
                                     [either mpp or spp] */
                            = cpu->csr.mstatus >> (xp == RV_PMACH ? 11 : 8) &
                              xp;
                        rv_u32 xpie /* previous ie bit */ =
                            rv_b(cpu->csr.mstatus, 4 + xp);
                        rv_u32 mprv /* modify privilege */ =
                            rv_b(cpu->csr.mstatus, 17);
                        if (rv_b(cpu->csr.mstatus, 22) && xp == RV_PSUPER)
                            return rv_trap(
                                cpu, RV_EILL, tval); /* exception if tsr=1 */
                        mprv *= yp == RV_PMACH;      /* if y != m, mprv' = 0 */
                        cpu->csr.mstatus &=
                            xp == RV_PMACH
                                ? 0xFFFDE777           /* {mpp, mie, mpie, mprv}
                                                          <- 0 */
                                : 0xFFFDFEDD;          /* {spp, sie, spie,
                                                          mprv} <- 0 */
                        ...
                        cpu->priv = yp;                     /* priv <- y */
                        cpu->next_pc =
                            xp == RV_PMACH ? cpu->csr.mepc : cpu->csr.sepc;
                    }
                    else if (rv_irs2(i) == 5 && rv_if7(i) == 8) { /*I wfi */
                        cpu->pc = cpu->next_pc;
                        return (err = rv_service(cpu)) == RV_TRAP_NONE
                                   ? RV_TRAP_WFI
                                   : err;
                    }
                    else if (rv_if7(i) == 9) { /*I sfence.vma */
                        if (cpu->priv == RV_PSUPER &&
                            (cpu->csr.mstatus & (1 << 20)))
                            return rv_trap(cpu, RV_EILL, tval);
                        cpu->tlb_valid = 0;
                    }
                    else if (!rv_irs1(i) && !rv_irs2(i) &&
                             !rv_if7(i)) { /*I ecall */
